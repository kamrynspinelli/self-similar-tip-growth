...
load('tip_variety_data_hyphoids_k_2_mu_2.mat');
a_h_nl_2 = a;
% b_h_nl_2 = b;
gam_peaks_h_nl_2 = gam_peaks;
ext_peaks_h_nl_2 = ext_peaks;
R0_h_nl_2 = R0;
RN_h_nl_2 = RN;
tip_lengths_h_nl_2 = tip_lengths;
ZN_h_nl_2 = ZN;
W_ints_h_nl_2 = W_ints;
W_h_nl_2 = W;
W_ints_ext_h_nl_2 = W_ints_ext;
W_ext_h_nl_2 = W_ext;
gams_h_nl_2 = gams;
L0Splines_h_nl_2 = L0Splines;
strainls_h_nl_2 = strainls;
kssplines_h_nl_2 = kssplines;
max_ks_h_nl_2 = max(kssplines, [], 2);
tip_ks_h_nl_2 = kssplines(:,end);
max_gams_h_nl_2 = max(gams, [], 2);
for i = 1:size(gams, 1)
    ks_peaks_h_nl_2(i) = strainls(i,end-find(fliplr(kssplines(i,:)) == max(fliplr(kssplines(i,:))))+2:end) * L0Splines(i,end-find(fliplr(kssplines(i,:)) == max(fliplr(kssplines(i,:))))+2:end)';
end
...
p = plot(R0_chloronema_nl_canonical ./ RN_chloronema_nl_canonical, gam_peaks_chloronema_nl_canonical ./ RN_chloronema_nl_canonical, '.', 'LineWidth', 2.0, 'MarkerSize', 20, 'Color', 'r');
...
% ylim([0 0.67]);
l = legend('Nonlinear K_h = \mu_h = 6', 'Nonlinear K_h = \mu_h = 24', ...
    'Nonlinear ellipsoid K_h = \mu_h = 6', 'Nonlinear ellipsoid K_h = \mu_h = 24', ...
    'Linear \nu = 0.3, E_h = 30', 'Linear \nu = 0, E_h = 30', ...
    'Root hair (fitted hyphoid)', 'Yeast (fitted ellpsoid)', 'Moss chloronema (fitted hyphoid)', 'Moss caulonema (fitted hyphoid)', ...
    'Location', 'northwest');
%     'Nonlinear K = \mu = 12', 'Linear \nu = 0.3, E = 60'
%     'Root hair (nonlinear K = \mu = 24)', 'Root hair (linear \nu = 0.3, E = 30)', ...
set(l.BoxFace, 'ColorType','truecoloralpha', 'ColorData',uint8(255*[.9;.9;.9;.5]));
...
% daspect([1 1 1]);
% set(ax,'XTick',[], 'YTick', []);
% ax = axes('Position',[.43 .38 .1 .1]); % inset plot 2: medium tip
% a = a_h_nl_24(9);
% h = base_height(a);
% plot(pi / a * (-h:0.001:h) .* cot(pi * (-h:0.001:h)) - 1/a, (-h:0.001:h), 'LineWidth', 2.0)
% xlim tight;
% daspect([1 1 1]);
% set(ax,'XTick',[], 'YTick', []);
% ax = axes('Position',[.72 .74 .1 .1]); % inset plot 3: flat tip
% a = a_h_nl_24(end);
% h = base_height(a);
% plot(pi / a * (-h:0.001:h) .* cot(pi * (-h:0.001:h)) - 1/a, (-h:0.001:h), 'LineWidth', 2.0)
% xlim tight;
% daspect([1 1 1]);
% set(ax,'XTick',[], 'YTick', []);
exportgraphics(gcf, 'tip_shape_variety_R0bar_lp.png', 'Resolution', 300);
close all;
...
p = plot(R0_chloronema_nl_canonical ./ RN_chloronema_nl_canonical, W_chloronema_nl_canonical ./ RN_chloronema_nl_canonical, '.', 'LineWidth', 2.0, 'MarkerSize', 20, 'Color', 'r');
...
% set(get(get(h,'Annotation'),'LegendInformation'), ... % hack to not display legend entry for the divider
%     'IconDisplayStyle','off');
exportgraphics(gcf, 'tip_shape_variety_R0bar_W.png', 'Resolution', 300);
close all;
