function [ Tl,Tr,rv,error,frames,strainFramesl,strainFramesr ] = equilibrium_newton_cspline_local( rv,LUT,L0,R0,K,MU,ext_verts,ext_force_status,L0Parabolic,R0Parabolic,KParabolic,MUParabolic)
...
%Find initial guess near the solution
    while error>Tol
        ...
        % [tX,X] = ode15s(@solver_cspline_local,[0 inc],X0,options,LUT,L0,R0,K,MU,ext_verts,ext_force_status);
    ...
    X = fsolve(@solver_cspline_local_fast,X0,options2,LUT,L0,R0,K,MU,ext_verts,ext_force_status);
    X0=X;
    frames(end+1, :) = X0;
    rv = reshape(X0',N,2)'; 
    [Ls Ltheta] = strains(rv, L0, R0);
    ...
    [Tl Tr] = tensions(Ls, Ltheta, K, MU); % column vectors
    Tl = Tl';
    Tr = Tr';
end
...
function [Tl Tr] = tensions(Ls, Ltheta, K, MU)
    Tl = 1/2 * MU' .* (Ltheta.^-2 - Ls.^-2) + K' .* (Ls .* Ltheta - 1);
    Tr = 1/2 * MU' .* (Ls.^-2 - Ltheta.^-2) + K' .* (Ls .* Ltheta - 1);
