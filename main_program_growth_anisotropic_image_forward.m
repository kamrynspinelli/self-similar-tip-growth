...
filename = "../cell-profiles/yeast-2.png";
name = 'yeast';
stiffness = 12;
...
for step = 2:num_frames
    % Compute secretion data using forward finite difference
    ...
    Gam(1) = 0;
    s(2) = L0Spline(end);
    Gam(2) = (strainl(end) - 1) * s(2);
    gam(1) = 1;
    gam_s(1) = strainl(end) - 1;
    gam_theta(1) = strainl(end) - 1;
    for i = 2:size(L0Spline, 1)-1
...
%         gam(i+1) = gam_theta(i+1) / (strainr(N-i+1) - 1);
        Gam(i+1) = Gam(i) * (1 + (strainl(end-i+1) - 1) / (strainr(end-i+1) - 1) ...
            * (rv(2,end-i) - rv(2,end-i+1)) / rv(2,end-i+1));
    end
    gam_s = diff(Gam) ./ diff(s);
    gam_s(end+1) = 0; % add an extra zero for the base of the cell
    gam = gam_s ./ (fliplr(strainl(2:end)) - 1);
    gam_theta = gam .* (fliplr(strainr(2:end)) - 1);
...
%     R0Parabolic = R0Parabolic + tstep * drds(2:end)' .* R0Parabolic .* fliplr([cumsum(fliplr(L0Spline(2:end)') .* gam_s(1:end-1)) 0])';
