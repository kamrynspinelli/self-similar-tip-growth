function [X_dot] = solver_cspline(t,X,LUT,L0,R0,K,MU,ext_verts,ext_force_status)
global P 
%X is a row vector
...
%     bisector(i) = (rv(1,i) + rv(1,i+1)) / 2;
    rmid(i) = horizA(i) * bisector(i)^3 + horizB(i) * bisector(i)^2 + horizC(i) * bisector(i) + horizD(i);
end
for i = cut:N-1
...
for index = 2:cut-1
    ...
    rC = horizC(index);
    rD = horizD(index);
    lSigmaS = sigmaS(index-1);
    rSigmaS = sigmaS(index);
    lSigmaTheta = sigmaTheta(index-1);
    rSigmaTheta = sigmaTheta(index);
...
rC = vertC(1);
rD = vertD(1);
lSigmaS = sigmaS(cut-1);
rSigmaS = sigmaS(cut);
lSigmaTheta = sigmaTheta(cut-1);
rSigmaTheta = sigmaTheta(cut);
...
% 2 * lSigmaS * midpt(@(x) (3 * lA * exp(-x)^2 + 2 * lB * exp(-x) + lC) /
% (sqrt(1 + (3 * lA * exp(-x)^2 + 2 * lB * exp(-x) + lC)^2)), -log(-lRmid),
% 1000, 1000000) seems to basically work for the integral
% Pvec
r_dot(:,N) = r_dot(:,N) + 2 * P * [-rv(2,N) - lRmid; 0];

r_dot(1,1) = 0; % clamp the first point to the r-axis
r_dot(2,end) = 0; % clamp the last point to the z-axis

% if the first or last points fall off the axis, blow up the error
% if abs(rv(1,1)) > 0.0001
%     r_dot(1,1) = 1000 * -sign(rv(1,1));
% end
% if abs(rv(2,end)) > 0.0001
%     r_dot(2,end) = 1000 * -sign(rv(2,end));
% end
