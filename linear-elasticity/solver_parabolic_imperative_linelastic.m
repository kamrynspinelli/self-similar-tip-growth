function [X_dot] = solver_parabolic_imperative_linelastic(t,X,LUT,L0,R0,POIS,YOUNG,ext_verts,ext_force_status)
global P 
%X is a row vector
...
ext_YOUNG = zeros(1, N+1); % preallocate extended patch parameter array
ext_YOUNG(1) = YOUNG(1); % fill in
ext_YOUNG(2:N) = YOUNG(:);
ext_YOUNG(N+1) = YOUNG(N-1);
ext_POIS = zeros(1, N+1);
ext_POIS(1) = POIS(1);
ext_POIS(2:N) = POIS(:);
ext_POIS(N+1) = POIS(N-1);
...
for i = 1:N-1 % Save the data for each patch, except the two tip-adjacent ones
    ...
    sigmaS(i) = ext_YOUNG(i) / (2 * (1 - ext_POIS(i))) .* (Ls + Ltheta - 2) + ...
        ext_YOUNG(i) ./ (2 * (1 + ext_POIS(i))) .* (Ls - Ltheta);
    sigmaTheta(i) = ext_YOUNG(i) / (2 * (1 - ext_POIS(i))) .* (Ls + Ltheta - 2) + ...
        ext_YOUNG(i) ./ (2 * (1 + ext_POIS(i))) .* (Ltheta - Ls);
end
% Upper tip patch
i = N;
q1 = ext_rv(:,N);
...
%     * sqrt(1 + 4*t^2 / lLambda^4);
% % compute the force
% lPart = [0; midpt(lIntegrand, lTmid, lTmax, 16)];
% rPart = [0; (rSigmaS - rSigmaTheta) * ((sqrt(4 * A^2 * rmid^2) - atanh(sqrt(4 * A^2 * rmid^2))) ...
...
% end
% % compute the force
% lPart = midpt(lIntegrand, lTmid, lTmax, 16);
