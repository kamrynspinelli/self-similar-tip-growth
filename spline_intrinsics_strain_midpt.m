function [L0 lR0 rR0] = spline_intrinsics(rv)
    %% Find the splines
...
%     daspect([1 1 1]);
    ...
    R0 = zeros(N-1,1);
    for i = 1:cut-1
        L0(i) = midpt(@(z) sqrt(1 + (3 * horizA(i) * z^2 + 2 * horizB(i) * z + horizC(i))^2), rv(1,i), rv(1,i+1), 16);
        ...
        lR0(i) = horizA(i) * lqrtrpt(i)^3 + horizB(i) * lqrtrpt(i)^2 + horizC(i) * lqrtrpt(i) + horizD(i);
        rR0(i) = horizA(i) * rqrtrpt(i)^3 + horizB(i) * rqrtrpt(i)^2 + horizC(i) * rqrtrpt(i) + horizD(i);
    end
    for i = cut:N-1
        L0(i) = midpt(@(r) sqrt(1 + (3 * vertA(i-cut+1) * r^2 + 2 * vertB(i-cut+1) * r + vertC(i-cut+1))^2), -rv(2,i), -rv(2,i+1), 16);
