function [X_dot] = solver(t,X,LUT,L0,R0,K,MU,ext_verts,ext_force_status)
global P 
%X is a row vector
...
rbL = LUTP*rv(2,:)'; % y-values at the left endpoint of each patch
rbR = LUTM*rv(2,:)'; % y-values at the right endpoint of each patch
...
rb(:,1) = rb(:,1)./D;
rb(:,2) = rb(:,2)./D; % director: unit vector in the direction of each patch
rbn = [rb(:,2) -rb(:,1)];%outward normal
rm = 0.5*rb1; % y-values at midpoints of each segment
angle = -atan(rb(:,2)./rb(:,1)); % angle between each segment and the horizontal
lnL = log(rbR./rm); % ln terms in equation (8)
lnR = log(rm./rbL);
