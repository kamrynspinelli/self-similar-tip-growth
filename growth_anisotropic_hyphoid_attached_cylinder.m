function [tip_length gam_peak ZN RN Rtip W_int gam L0Spline strainl ksspline] = growth_anisotropic_hyphoid_attached_cylinder(a,stiffness)
% clear all
time = cputime;
%% parameters
...
for i = 0:3*N/4
    target_arclength = total_arclength * i / (3*N/4);
    yl = 0;
    yr = yr_init;
    mid = (yl+yr)/2;
    fmid = f(mid);
    while abs(midpt(@(y) sqrt(1 + fp(y).^2), 0, mid, 2048) - target_arclength) > total_arclength / (3*N/4) * 0.001
    ...
    rv(1,(3*N/4)+1-i) = f(mid);
    rv(2,(3*N/4)+1-i) = mid;
end

rv_ext(1,:) = [(-5-3):3/(N/4):(-5-3/(N/4)) rv(1,:)]; % attach a cylindrical pipe to the shank
rv_ext(2,:) = [max(rv(2,:))*ones(1,N/4) rv(2,:)];
rv_ext(1,end) = 1/a;
rv_ext(2,end) = 0;
rv_ext(1,:) = rv_ext(1,:) - min(rv_ext(1,:));
rv = rv_ext;
...
save(['hyphoid_attached_cylinder_a_', num2str(a), '_k_', num2str(stiffness), '_mu_', num2str(stiffness), '.mat'], ...
...
exportgraphics(gcf, ['media/anisotropic_growth_gamma_s_theta_hyphoid_attached_cylinder_a_', num2str(a), '_k_', num2str(k), '_mu_', num2str(mu), '.png']);
close all;
...
xlabel("s");
ylabel("\gamma(s)");
exportgraphics(gcf, ['media/anisotropic_growth_gamma_hyphoid_attached_cylinder_a_', num2str(a), '_k_', num2str(k), '_mu_', num2str(mu), '.png']);
close all;
...
exportgraphics(gcf, ['media/anisotropic_strain_distribution_hyphoid_attached_cylinder_a_', num2str(a), '_k_', num2str(k), '_mu_', num2str(mu), '.png']);
close all;
...
p.Color = 'black';
exportgraphics(gcf, ['media/anisotropic_monotonicity_hyphoid_attached_cylinder_a_', num2str(a), '_k_', num2str(k), '_mu_', num2str(mu), '.png']);
close all;
...
pause(0.5);
exportgraphics(gcf, ['media/anisotropic_profile_overlaid_hyphoid_attached_cylinder_a_', num2str(a), '_k_', num2str(k), '_mu_', num2str(mu), '.png']);
close all;
...
ax.XRuler.TickLabelGapOffset = -20;
pause(0.5);
exportgraphics(gcf, ['media/anisotropic_profile_overlaid_strain_rates_hyphoid_attached_cylinder_a_', num2str(a), '_k_', num2str(k), '_mu_', num2str(mu), '.png']);
close all;
