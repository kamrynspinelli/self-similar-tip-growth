function [tip_length gam_peak ZN RN Rtip W_int gam L0Spline strainl] = growth_anisotropic_rojas_data(profile_num, stiffness)
...
%% Initialize cells
load('../cell-profiles/rojas-pollen-tube-processed.mat'); % load the processed data from Rojas
rv = rvs{profile_num};
...
save(['../cell-profiles/rojas-pollen-tube-', num2str(profile_num), '.mat'], ...
...
exportgraphics(gcf, ['media/anisotropic_growth_gamma_s_theta_rojas-pollen-tube-', num2str(profile_num), '_k_', num2str(k), '_mu_', num2str(mu), '.png']);
close all;
...
xlabel("s");
ylabel("\gamma(s)");
exportgraphics(gcf, ['media/anisotropic_growth_gamma_rojas-pollen-tube-', num2str(profile_num), '_k_', num2str(k), '_mu_', num2str(mu), '.png']);
close all;
...
exportgraphics(gcf, ['media/anisotropic_growth_epsilon_rojas-pollen-tube-', num2str(profile_num), '_k_', num2str(k), '_mu_', num2str(mu), '.png']);
close all;
...
exportgraphics(gcf, ['media/anisotropic_strain_distribution_rojas-pollen-tube-', num2str(profile_num), '_k_', num2str(k), '_mu_', num2str(mu), '.png']);
close all;
...
p.Color = 'black';
exportgraphics(gcf, ['media/anisotropic_monotonicity_rojas-pollen-tube-1_k_', num2str(k), '_mu_', num2str(mu), '.png']);
close all;
...
pbaspect([1.7 1 1]);
ax.XAxis.Visible = 'off'; % turn off this x-axis
set(get(ax,'YLabel'),'Position',get(get(ax,'YLabel'),'Position') + [0.14 0 0])
...
pbaspect([1.7 1 1]);
exportgraphics(gcf, ['media/anisotropic_profile_overlaid_rojas-pollen-tube-', num2str(profile_num), '_k_', num2str(k), '_mu_', num2str(mu), '.png']);
close all;
...
function profile_plots(Px, Py)
    ...
    daspect([1 1 1]);
    axis off;
    exportgraphics(gcf, 'media/root-hair-profiles.png');
end