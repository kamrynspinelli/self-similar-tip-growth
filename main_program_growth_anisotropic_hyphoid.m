...
target_length = 1.1 * rv(1,end); % grow until the tube has elongated to twice the pre-growth length
...
%     X = fsolve(@solver_cspline_local_fast,X0,options2,LUT,L0,R0,K,MU,ext_verts,0);
    X = newt_vec(@solver_cspline_local_fast, X0, TolFun * 4, TolX * 4, LUT,L0,R0,K,MU,ext_verts,0);
    res = reshape(X',N,2)'; 
end