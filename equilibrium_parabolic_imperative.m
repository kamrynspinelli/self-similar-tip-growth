...
%Find initial guess near the solution
    while error>Tol 
        [tX,X] = ode45(@solver_parabolic_imperative,[0 inc],X0,options,LUT,L0,R0,K,MU,ext_verts,ext_force_status); % parabolic arcs
    ...
    X = fsolve(@solver_parabolic_imperative_fast,X0,options2,LUT,L0,R0,K,MU,ext_verts,ext_force_status); % parabolic arcs
    X0=X;
    frames(end+1, :) = X0;
    rv = reshape(X0',N,2)';  
    currentArcs = ParabolicArc.all_arcs(rv, K, MU, L0, R0);
