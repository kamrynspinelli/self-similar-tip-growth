...
%         [tX,X] = ode45(@solver_cspline_local,[0 inc],X0,options,LUT,L0,R0,K,MU,ext_verts,ext_force_status);
    ...
    rvs{exp} = rv;
    err1(exp) = norm(rv - rv_canonical(:,1:2^(pow_max-pow(exp)):end), 1);
    err2(exp) = norm(rv - rv_canonical(:,1:2^(pow_max-pow(exp)):end), 2);
    errInf(exp) = norm(rv - rv_canonical(:,1:2^(pow_max-pow(exp)):end), Inf);
end

loglog(2.^pow(1:end-1), err1, 2.^pow(1:end-1), err2, 2.^pow(1:end-1), errInf);
...
legend(['1-norm, slope = ', num2str((log(err1(end)) - log(err1(end-1))) ./ log(2))], ...
    ['2-norm, slope = ', num2str((log(err2(end)) - log(err2(end-1))) ./ log(2))], ...
    ['\infty-norm, slope = ', num2str((log(errInf(end)) - log(errInf(end-1))) ./ log(2))]);
exportgraphics(gcf, 'media/positional-error-analysis-sphere-alt.png')
